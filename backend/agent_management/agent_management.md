...
active ◄──────────────────────────────────┬
    │                                     │
    ▼ 90s no heartbeat                    │ Heartbeat restored (from degraded OR unreachable)
degraded ──────────────────────────────┘
    │
    ▼ 10 min no heartbeat
[cluster status: degraded]
    │
    ▼ 30 min no heartbeat
unreachable ──────────────────────────► active (heartbeat restored)
...
> **Crash recovery — new cert never confirmed:** If the platform does not receive a heartbeat authenticated with the new certificate within `AGENT_CERT_TRANSITION_WINDOW_HOURS` (default: 48 hours) after issuing it, the new certificate is automatically retired (`is_active` set to `false`). The old certificate remains valid. The platform emits a `cert.rotation_abandoned` internal log event.
>
> **Continuous pending-cert check (agent-side):** The agent checks for pending-unconfirmed certificates on every token rotation check cycle (the same 24-hour interval used to detect approaching expiry), not only on startup. If a pending-unconfirmed certificate is found (issued but never confirmed via a new-cert heartbeat), the agent re-attempts the confirmation heartbeat with the pending certificate. If this fails after 3 attempts with 10-minute spacing, the pending certificate is discarded and a new rotation is initiated from step 1.
...
- **Single-use enforcement:** After the first successful agent registration, the `cluster_tokens` row has its `consumed_at` column set to the current timestamp. Any subsequent registration attempt using the same token receives HTTP 401 `TOKEN_ALREADY_USED`. This prevents an attacker who intercepts the token from registering a rogue agent after the legitimate agent has already registered.
...
| `AGENT_CERT_TRANSITION_WINDOW_HOURS` | `48` | Hours within which the agent must send a heartbeat authenticated with the newly issued certificate before the new cert is auto-retired. |
...
- **Late job guard:** The heartbeat monitor job computes the expected window as `NOW() - (AGENT_HEARTBEAT_MONITOR_INTERVAL_SECONDS * 2)`. If `agents.last_heartbeat_at` is within this window, the check passes regardless of how late the BullMQ job ran. This prevents false positive `agent.heartbeat_missed` events from delayed job execution.
