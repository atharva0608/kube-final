...
| `PATCH` | `/api/v1/workloads/:id/config` | JWT (ADMIN/MEMBER) | Upsert workload configuration. Accepts partial updates (only provided fields are changed). Implicitly sets `review_status = confirmed` on the associated `review_items` row. Returns the complete updated `workload_config` object. **Atomic config + review update:** The `PATCH /workloads/:id/config` handler executes both writes in a single database transaction: (1) UPSERT `workload_config` with new field values, (2) `UPDATE review_items SET status='confirmed', reviewed_at=NOW(), reviewed_by=$user_id WHERE (review_id, workload_id) = ($1, $2) AND status='pending'`. If the transaction rolls back for any reason, neither write is applied. Because both writes are in the same transaction, the `POST /clusters/:id/reviews/:review_id/complete` endpoint's check of `review_items.status` will never see an inconsistent intermediate state. |
...
|---|---|---|
| `REVIEW_AUTO_COMPLETE_AFTER_HOURS` | `0` (disabled) | If > 0, automatically complete a pending review this many hours after creation (for orgs that want fully automated pipelines). **Auto-complete exclusion constraint:** When `REVIEW_AUTO_COMPLETE_AFTER_HOURS` triggers auto-completion, any `review_items` rows still in `pending` where `business_criticality` has never been set are marked with `excluded=true` on their `workload_config` row rather than being assigned a default criticality level. Setting `excluded=true` removes the workload from Spot migration scope entirely until an operator explicitly sets `business_criticality` and clears the exclusion. An operator notification is sent listing all auto-excluded workloads with the message: 'These workloads were excluded from migration because their criticality was not set before the review deadline. To include them, set business_criticality and set excluded=false via PATCH /workloads/:id/config.' **Rationale:** assigning STANDARD criticality would cause these workloads to execute first in Phase 4 ordering. If the workload is actually CRITICAL, this is the dangerous outcome. Exclusion is the safe choice. |
